...
const generateStaffUrl = (sessionKey: string) => {
...
router.post('/register-pd', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const userResult = await client.query(
      `INSERT INTO users (username, email, password_hash, role, stream_key, is_pd) 
       VALUES ($1, $2, $3, $4, $5, $6) 
       RETURNING id, username, email, role, stream_key, created_at`,
      [username, email, passwordHash, 'pd', streamKey, true]
    );
    ...
    const sessionResult = await client.query(
      ...
      [`pd_${sessionKey}`, `${username}'s Studio`, newUser.id]
    );
    ...
    await client.query(
      `INSERT INTO session_participants (session_id, user_id, role) 
       VALUES ($1, $2, $3)`,
      [session.id, newUser.id, 'pd']
    );

    // Register PD software if flag is set
    if (isPDSoftware) {
      await client.query(
        `INSERT INTO pd_software_registrations 
         (user_id, software_version, ip_address, vmix_port) 
         VALUES ($1, $2, $3, $4)`,
        [newUser.id, req.body.softwareVersion || '1.0.0', req.ip, req.body.vmixPort || 8088]
      );
    }
...
router.get('/registration-url', asyncHandler(async (req: Request, res: Response) => {
...
router.post('/login-pd', asyncHandler(async (req: Request, res: Response) => {
  ...
  // Update last login
  await pool.query(
    'UPDATE users SET last_pd_login = CURRENT_TIMESTAMP WHERE id = $1',
    [user.id]
  );
...
router.get('/stream-info', asyncHandler(async (req: Request, res: Response) => {
