...
router.post('/register', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    ...
    }, process.env.JWT_SECRET, { expiresIn: '30d' } // Long-lived token for PD software
    );
    res.json({
        streamKey,
        pdToken,
...
router.get('/stream-config/:sessionKey', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const { sessionKey } = req.params;
    const userId = req.user.id;
    // Verify session ownership or PD role
    const sessionResult = await db_1.default.query(`SELECT s.*, sp.role as user_role
     FROM sessions s
     LEFT JOIN session_participants sp ON s.id = sp.session_id AND sp.user_id = $2
     WHERE s.session_key = $1`, [sessionKey, userId]);
...
router.post('/inputs/:sessionKey', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const { sessionKey } = req.params;
    const { inputs, timestamp, vmixVersion } = req.body;
    const userId = req.user.id;
    // Verify PD role
    const sessionResult = await db_1.default.query(`SELECT s.id, sp.role
     FROM sessions s
     JOIN session_participants sp ON s.id = sp.session_id
     WHERE s.session_key = $1 AND sp.user_id = $2`, [sessionKey, userId]);
    ...
    await db_1.default.query(`INSERT INTO stream_configs (session_id, input_list, vmix_version, updated_at)
     VALUES ($1, $2, $3, NOW())
     ON CONFLICT (session_id) 
     DO UPDATE SET 
       input_list = EXCLUDED.input_list,
       vmix_version = EXCLUDED.vmix_version,
       updated_at = NOW()`, [sessionId, JSON.stringify(inputs), vmixVersion || 'unknown']);
...
router.post('/tally/:sessionKey', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const { sessionKey } = req.params;
    ...
    await db_1.default.query(`INSERT INTO stream_configs (session_id, tally_program, tally_preview, input_list, updated_at)
     VALUES ($1, $2, $3, $4, NOW())
     ON CONFLICT (session_id)
     DO UPDATE SET 
       ...
       input_list = EXCLUDED.input_list,
       updated_at = NOW()`, [sessionId, programInput, previewInput, JSON.stringify(inputs || {})]);
...
router.get('/vmix-config', auth_1.authenticateToken, asyncHandler(async (req, res) => {
    const userId = req.user.id;
    // Check PD role
    const userResult = await db_1.default.query('SELECT role, stream_key FROM users WHERE id = $1', [userId]);
    if (userResult.rows[0]?.role !== 'pd' && userResult.rows[0]?.role !== 'admin') {
...
router.get('/health', asyncHandler(async (req, res) => {
    const dbHealthy = await db_1.default.query('SELECT 1').then(() => true).catch(() => false);
    res.json({
        status: 'healthy',
